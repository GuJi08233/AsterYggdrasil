...
| 能力封禁 | 按 scope 限制用户使用 Yggdrasil、profile、材质上传和公共材质库交互能力 |
...
能力封禁用于限制某个用户能否使用特定功能，不会禁用整个账号，也不会只作用于单个 Minecraft profile。
...
一条封禁记录可以包含多个 scope。用户账户页默认只展示当前有效的能力限制，不展示已撤销或已过期记录。详细 scope 和错误码见 [能力封禁](/guide/user-bans)。
